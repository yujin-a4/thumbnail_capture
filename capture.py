...
# 3. 메인 화면: 파일 업로드 영역
uploaded_file = st.file_uploader("엑셀 파일을 업로드하세요 (urls_and_texts.xlsx)", type=["xlsx"])
    ...
    if st.button("🚀 캡처 작업 시작"):
        ...
        with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
            for index, row in df.iterrows():
                ...
                status_container.write(f"⏳ **{file_name}** 처리 중... ({index+1}/{len(df)})")
